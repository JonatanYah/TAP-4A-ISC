...
public class tabla extends JPanel {
	...
	public void setup() {
		...
		for (int i = 0; i < barcoArray.length; i++){
			int temp = barcoArray[i].getpiezas().length;
		...
		for (int j = 0; j < barcoArray.length; j++) {
			...
			for (int i = 0; i < barcoArray[j].getpiezas().length; i++) {
				ImageIcon icon = new ImageIcon(barcoArray[j].getpiezas()[i].getImagen());
			...
			panel.setSize(barcoArray[j].getpiezas().length * (TILE_SIZE + BORDER_SIZE), TILE_SIZE);
	...
	private void removeShipFromGridArray(Barco ship, boolean isVertical) {
		for (int i = 0; i < gridArray.length; i++) {
			for (int j = 0; j < gridArray[i].length; j++) {
				for (int k = 0; k < ship.getpiezas().length; k++) {
					if (gridArray[j][i] == (BarcoP) ship.getpiezas()[k]) {
						gridArray[j][i] = 1;
					}
				}
			}
	...
	private void addShipToGridArray(Barco ship, Point location, boolean isVertical) {
		if (location.getX() < gridArray.length && location.getX() >= 0 && location.getY() < gridArray.length
				&& location.getY() >= 0) {
			for (int i = 0; i < ship.getpiezas().length; i++) {
				if (isVertical) {
					gridArray[(int) location.getX()][(int) location.getY() + i] = ship.getpiezas()[i];
				} else {
					gridArray[(int) location.getX() + i][(int) location.getY()] = ship.getpiezas()[i];
				}
			}
		}
	}
